...
var App = {
	...
	formAjaxSubmit : function(formId, url, method, callback) {
		var timeout = setTimeout(this.bodyRefreshImg, 2000);
...
/**
 * 在datatables列表中显示数据字典对应的内容
 * 
 * @param codes
 * @param value
 * @returns
 */
function displayDataContent(codes, value) {
	for (var i = 0; i < codes.length; i++) {
		if (value == codes[i].dataValue) {
			return codes[i].dataContent;
		}
	}
}
